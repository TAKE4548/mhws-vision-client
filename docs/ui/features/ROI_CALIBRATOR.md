...
現在は以下の **6段階** ガイド付きウィザード構成です。

| Step | ID | 概要 |
|:---:|:---|:---|
| 0 | `setup` | プロファイル選択または新規作成 |
| 1 | `source` | 解析の基準となる動画/フレームの指定 |
| 2 | `parent` | 護石情報が表示される全体領域 (Parent Window) の指定 |
| 3 | `items` | 各項目 (Rarity, Slots, Skills) の位置微調整 |
| 4 | `normalization` | スロット判定用の色の基準点を指定 |
| 5 | `save` | プロファイル名の入力と保存 |

---

## 3. 同期（等間隔配置）仕様
`items` ステップにおいて、効率的な配置のために同期（連動）機能が提供されます。

### 3-1. 同期グループの独立性
以下の 4 つのカテゴリは完全に独立して管理されます。
- **Slot Icons**: `slots[0..2].icon`
- **Slot Level Borders**: `slots[0..2].level`
- **Skill Names**: `skills[0..2].name`
- **Skill Level Borders**: `skills[0..2].level`

### 3-2. 連動ルール
各カテゴリ内の 1 つ目の要素（インデックス 0）を動かした際、同一カテゴリ内の他の 2 つの要素が、設定された `Gap`（間隔）を維持して自動的に追従します。カテゴリを跨いだ連動（例：アイコンを動かしてレベル枠が動く）は発生しません。
