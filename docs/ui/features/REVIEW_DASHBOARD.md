...
- 護石画像（Crops）の解決は、バックエンドから返却される相対パス（例: `/assets/crops/...`）を **Single Source of Truth (SSoT)** とする。
- フロントエンド側では、`resolveImageUrl` ユーティリティを使用して `API_HOST` を動的に付与することで、デプロイ環境の変化や開発サーバーのポート変更に柔軟に対応する。
- 読み込みの堅牢性を確保するため、`VisionImage` コンポーネントによる自動リトライおよびキャッシュバスター（`?t=...`）の適用を必須とする。
